...
public class RemoveDuplicatesFromSortedArray {
	 ...
	 * Example 1:
	 * 
	 * Input: nums = [1,1,2] Output: 2, nums = [1,2] Explanation: Your function
	 ...
	 * Constraints:
	 * 
	 * 0 <= nums.length <= 3 * 104 -104 <= nums[i] <= 104 nums is sorted in
	 * ascending order.
	 */
    ...
    public static int removeDuplicates(int[] nums) {
        ...
        for (int i =1 ; i< nums.length; i++) {
            if(nums[i] != nums[prevNumIndex]) {
                int temp = nums[i];
                nums[i] = nums[prevNumIndex+1];
                nums[prevNumIndex+1] = temp;
                prevNumIndex = prevNumIndex+1;
            }
        }
        return prevNumIndex+1;
    }
